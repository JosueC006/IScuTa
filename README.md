<h1 align="center">ISCuTa</h1>

<p align="center">
  <img src="https://img.shields.io/badge/programa_de_inovação-recife_resolve-blue?style=for-the-badge" />
  <img src="https://img.shields.io/badge/status-em_desenvolvimento-orange?style=for-the-badge" />
</p>

<blockquote>
A prevenção começa na informação — informe-se, ajude-se, aplicação orientada a Acessibilidade, Segurança e ao Acolhimento.
</blockquote>

---

<h2>📝 Sobre o Projeto</h2>

IScuTa é uma healthtech que combate ISTs (HIV, sífilis) entre jovens de 15 a 34 anos com uma jornada de prevenção gamificada e privada. Ensina sobre PrEP e PEP via vídeos e quizzes, tem chatbot confidencial, geolocalização para postos de saúde e agenda de exames. Estudos do PubMed apontam +40% de adesão com gamificação, aproximando o jovem do SUS.
